...
namespace Helpers
{
	public class MPEGInfo
    {
        ...
        static public int FramesCount(String sFilename)
        {
            MPEGInfo cMPEGInfo = new MPEGInfo(sFilename);
            cMPEGInfo.Parse(); 
            //return (int)(cMPEGInfo.VideoInfo.Frames);
            return (int)((cMPEGInfo.VideoInfo.Duration + cMPEGInfo._initialTS) * cMPEGInfo.VideoInfo.FrameRate);
        }
        ...
        private string[] ModeIndex = new string[4] // these can change anytime specs change so be careful
        ...
        private string[] EmphasisIndex = new string[4] // same as above
        ...
        private bool EnableTrace = false; // debugging purposes
        ...
        long _backwardBufferStart = 0;

        long _backwardBufferEnd = 0;



        byte GetByteBackwards(long offset)
        {
        ...
        long _forwardBufferStart = 0;

        long _forwardBufferEnd = 0;
        ...
        byte GetByte(long offset)
        {
            ...
            if (offset >= _forwardBufferEnd || offset < _forwardBufferStart) // need new buffer
            {
                ...
                if (pos > _fileSize) { pos = _fileSize; }
        ...
        long FindMarkerBackwards(long offset, byte marker)
        {
        ...
        void ParseSystem(int offset)
        {
            ...
            */
            ...
            long lastPack = FindMarkerBackwards(_fileSize - 13, 0xBA);
            ...
            else
            {
                ...
                lastPack = FindMarkerBackwards(_fileSize - 8, 0xB8);
                ...
                if (((GetByte(lastPack + 4) & 0xC0) == 0x40) || _mpeg2Found)
                {
                    ...
                    lastPack = FindMarkerBackwards(_fileSize - 8, 0xBA);
        ...
        double ReadTSMpeg2(long offset)
        {
    ...
    /*
        public void CountVideoFrames() // slow, find a better way of doing this
        {
